...
public class MongoDBManager : MonoBehaviour
{
    ...
    IEnumerator SignupCoroutine()
    {
        ...
        // Check if password is at least 8 characters with at least one uppercase letter, one number, and one special character
    ...
    IEnumerator LoginCoroutine()
    {
        ...
        using (UnityWebRequest www = new UnityWebRequest(serverUrl + "/login", "POST"))
        {
            ...
            else if (www.responseCode == 400) // Bad request
            {
                string responseText = www.downloadHandler.text;
                Debug.Log("Server Response: " + responseText);
